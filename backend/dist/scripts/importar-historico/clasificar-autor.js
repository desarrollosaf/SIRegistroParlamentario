...
exports.clasificarAutor = clasificarAutor;
/**
...
function clasificarAutor(autorOriginal, catalogo) {
    const texto = autorOriginal.trim();
    for (const regla of REGLAS) {
        if (regla.test.test(texto)) {
            const encontrado = catalogo.proponentesPorNombre.get((0, catalogos_1.normalizarNombre)(regla.proponente));
            if (encontrado) {
