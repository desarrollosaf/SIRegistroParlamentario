...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const r of conMatch) {
            ...
            const yaExiste = yield inciativas_puntos_ordens_1.default.findOne({ where: { id_punto: puntoId } });
            if (yaExiste) {
                yaExistian++;
                if (!yaExiste.folio_historico) {
                    yield yaExiste.update({ folio_historico: Number(r.folio) });
                }
                continue;
            }
            try {
                ...
                if (comisionIds.length > 0) {
                    yield puntos_comisiones_1.default.create({ id_punto: puntoId, id_comision: `[${comisionIds.join(',')}]` });
                    yield puntos_ordens_1.default.update({ se_turna_comision: 1 }, { where: { id: puntoId } });
                }
                creadas++;
            }
            catch (err) {
