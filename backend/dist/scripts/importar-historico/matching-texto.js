...
exports.tokenizarSignificativo = tokenizarSignificativo;
exports.puntuarCoincidencia = puntuarCoincidencia;
exports.tokenizarBigramas = tokenizarBigramas;
exports.puntuarBigramas = puntuarBigramas;
exports.calcularIDF = calcularIDF;
exports.puntuarConIDF = puntuarConIDF;
exports.puntuarCombinado = puntuarCombinado;
exports.elegirMejorCandidato = elegirMejorCandidato;
...
function tokenizarSignificativo(valor) {
 ...
 */
function puntuarCoincidencia(textoExcel, textoPunto) {
 ...
 */
function tokenizarBigramas(valor) {
    const tokens = normalizarTexto(valor).split(' ').filter(Boolean);
    const bigramas = new Set();
    for (let i = 0; i < tokens.length - 1; i++)
        bigramas.add(`${tokens[i]}_${tokens[i + 1]}`);
    return bigramas;
}
function puntuarBigramas(textoExcel, textoPunto) {
 ...
 */
function calcularIDF(candidatos) {
 ...
 */
function puntuarConIDF(textoExcel, textoPunto, idf) {
    const a = tokenizarSignificativo(textoExcel);
    const b = tokenizarSignificativo(textoPunto);
    if (a.size === 0 || b.size === 0)
        return 0;
    const peso = (t) => { var _a; return (_a = idf.get(t)) !== null && _a !== void 0 ? _a : 1; };
    let pesoA = 0;
    for (const t of a)
        pesoA += peso(t);
    let pesoB = 0;
 ...
 * redacción distinta.
 */
function puntuarCombinado(textoExcel, textoPunto, idf) {
 ...
 */
function elegirMejorCandidato(textoExcel, candidatos, umbralScore = 0.35, umbralMargen = 0.08) {
    ...
    if (mejor.score < umbralScore)
        return null;
    if (puntuados.length > 1 && margen < umbralMargen)
        return null;
