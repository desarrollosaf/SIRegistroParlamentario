...
async function main() {
  ...
  const inisBackup = leerTablaDeDump(rutaDump, 'inciativas_puntos_ordens', COL_INI);
  const presentaBackup = leerTablaDeDump(rutaDump, 'iniciativas_presenta', COL_PRESENTA);

  const iniPorSap = new Map<number, string>(); // folio -> backup ini.id
  for (const i of inisBackup) if (i.id_sap && i.id) iniPorSap.set(Number(i.id_sap), i.id);

  const presentaPorIniBackup = new Map<string, typeof presentaBackup>();
  for (const p of presentaBackup) {
    if (!p.id_iniciativa) continue;
    if (!presentaPorIniBackup.has(p.id_iniciativa)) presentaPorIniBackup.set(p.id_iniciativa, []);
  ...
  for (const ini of sembradas as any[]) {
    ...
    if (commit) {
      await IniciativasPresenta.destroy({ where: { id_iniciativa: ini.id } as any, force: true });
