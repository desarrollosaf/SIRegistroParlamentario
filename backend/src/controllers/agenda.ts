...
async function procesarDiputadosAsociadosComision(dipasociados: any[]): Promise<any[]> {
  if (!dipasociados || dipasociados.length === 0) return [];
...
export const catalogos = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        const expedPuntos = origenIdsType2.size > 0
          ...
          : [];
        const sesionIdsByExpediente = new Map<string, string[]>();
        for (const ep of expedPuntos as any[]) {
          if (!sesionIdsByExpediente.has(ep.expediente_id)) sesionIdsByExpediente.set(ep.expediente_id, []);
          ...
          for (const est of (d.puntosestudiados ?? [])) {
            if (String(est.type) === '1') {
              ...
              for (const sesId of (sesionIdsByExpediente.get(est.punto_origen_id) ?? [])) {
                idsIniciativas.push(...(iniByPunto2.get(sesId) ?? []));
              }
            }
          }
          return {
  ...
  try {
    ...
    const puntos = await Promise.all(puntosRaw.map(async punto => {
      ...
      const reservasConInfo = await Promise.all(
        (data.reservas || []).map(async (reserva: any) => {
          const { proponentesString, presentaString } = reserva.presentan?.length
            ? await procesarPresentan(reserva.presentan)
            : { proponentesString: '', presentaString: '' };
...
export const getreservas = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const reservasRaw = await TemasPuntosVotos.findAll({ 
      where: { id_punto: id },
      attributes: ["id", "tema_votacion"],
      include: [
        {
    ...
    if (body.tipo_evento != 0 && punto.evento.tipo_evento_id != "a413e44b-550b-47ab-b004-a6f28c73a750" ) {
     ...
     console.log("DICTAMENES ARRAY: ") 
      ...
      if (dictamenesArray.length === 1) {
        ...
        const prevConjunto = await IniciativaEstudio.findOne({
        ...
        if (prevConjunto) {
          ...
          expediente = await Expediente.findByPk(prevConjunto.punto_origen_id ?? '');
        ...
        if (!expediente) {
          ...
          await expediente.update({ descripcion: "Iniciativas en conjunto" });
        }
...
export const addDipLista = async (req: Request, res: Response): Promise<any> => {
  try {
    const { body } = req;
    const cargo = await TipoCargoComision.findOne({
      where: { id: body.id_cargo_dip }
    });

    if(cargo && cargo.valor == "Diputado Asociado"){
        const asistencia = await DiputadosAsociados.create({
...
export const EliminardipAsociado = async (req: Request, res: Response): Promise<any> => {
  try {
    const { id } = req.params;

    const integrante = await DiputadosAsociados.findOne({ where: { id } });
...
export const generarPDFVotacion = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const checkPageBreakVot = (y: number, neededHeight: number = 40) => {
    ...
    const drawSectionHeaderVot = (label: string, x: number, y: number, w: number): number => {
...
function generarDetalleSesion(
  ...
  doc.fontSize(11).font('Helvetica-Bold').fillColor('#fff')
    .text('DETALLE DE VOTACIÓN', 30, titY + 5,
  ...
  votosOrdenados.forEach((voto, index) => {
    if (currentY > 700) {
      ...
      doc.fontSize(11).font('Helvetica-Bold').fillColor('#fff')
        .text('DETALLE DE VOTACIÓN', 30, rY + 5,
          { width: doc.page.width - 60, align: 'center' });
      currentY = rY + 30;
...
function generarDetalleComision(
  doc: any, 
  votos: any[], 
  drawBackground: () => void, 
  getColorSentido: (sentido: number) => string
  ...
  doc.rect(30, titY, doc.page.width - 60, 22).fill('#96134b');
  doc.fontSize(11).font('Helvetica-Bold').fillColor('#fff')
    .text('DETALLE DE VOTACIÓN POR COMISIÓN', 30, titY + 5,
      { width: doc.page.width - 60, align: 'center' });
  doc.y = titY + 30;
  ...
  const comisionesOrdenadas = Object.values(votosPorComision)
    .sort((a: any, b: any) => a.importancia - b.importancia);
  ...
  comisionesOrdenadas.forEach((comision: any) => {
    if (doc.y > 650) {
      ...
      doc.rect(30, rY, doc.page.width - 60, 22).fill('#96134b');
      doc.fontSize(11).font('Helvetica-Bold').fillColor('#fff')
        .text('DETALLE DE VOTACIÓN POR COMISIÓN', 30, rY + 5,
          { width: doc.page.width - 60, align: 'center' });
      doc.y = rY + 30;
    }
    ...
    doc.fontSize(9).font('Helvetica-Bold').fillColor('#fff')
      .text(comision.nombre.toUpperCase(), 35, subY + 5,
    ...
    votosOrdenados.forEach((voto, index) => {
      if (currentY > 700) {
        ...
        doc.fontSize(9).font('Helvetica-Bold').fillColor('#fff')
          .text(`${comision.nombre.toUpperCase()} (continuación)`, 35, currentY + 5,
            { width: doc.page.width - 70, align: 'center' });
        currentY += 20;
...
export const enviarWhatsVotacionPDF = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    let iniciativasInfo:    any[] = [];
    let estudiosInfo:       any[] = [];

    // compartido
    let dictamenInfo: any = null;
    const esExpediente = estudio?.type === "2";
    ...
    if (estudio) {
      ...
      if (estudio.type === "1") {
        ...
        if (puntoOrigen) {
          sesionInfo = {
            fecha:       puntoOrigen.evento?.fecha ? new Date(puntoOrigen.evento.fecha).toLocaleDateString('es-MX') : 'N/A',
        ...
        if (dictamen) {
          ...
          const comisionesData = await getComisionesTurnado(puntoOrigenId);
          let autoresString = '';
          if (puntoDict?.id) {
            const iniciativaDictamen = await IniciativaPuntoOrden.findOne({
              ...
              attributes: ["id"],
              include: [{
                model: IniciativasPresenta, as: "presentan",
                attributes: ["id_tipo_presenta", "id_presenta"],
                include: [{ model: Proponentes, as: "tipo_presenta", attributes: ["id", "valor"] }]
              }]
            });
            if (iniciativaDictamen?.presentan?.length > 0) {
              const resultado = await procesarPresentan(iniciativaDictamen.presentan);
        ...
        const puntosOrigenIds = expPuntos
          .map((e: any) => e.punto_origen_sesion_id)
          .filter(Boolean);

        console.log("PUNTOS ORIGEN IDS:", puntosOrigenIds);

        if (puntosOrigenIds.length > 0) {
          const puntosOrigen = await PuntosOrden.findAll({
            where: { id: { [Op.in]: puntosOrigenIds } },
            include: [{
              model: Agenda, as: 'evento',
              attributes: ["id", "fecha", "descripcion"],
              include: [{ model: TipoEventos, as: 'tipoevento', attributes: ["nombre"] }]
          ...
          sesionesOrigenInfo = puntosOrigen.map((po: any) => ({
            nopunto:     po.nopunto ?? 'N/A',
            punto:       po.punto   ?? 'N/A',
            fecha:       po.evento?.fecha ? new Date(po.evento.fecha).toLocaleDateString('es-MX') : 'N/A',
            descripcion: po.evento?.descripcion ?? 'N/A',
            tipo_evento: po.evento?.tipoevento?.nombre ?? 'N/A'
          }));

          const iniciativas = await IniciativaPuntoOrden.findAll({
            where: { id_punto: { [Op.in]: puntosOrigenIds } },
          ...
          const todosLosExpedientes = await ExpedienteEstudiosPuntos.findAll({
            where: { punto_origen_sesion_id: { [Op.in]: puntosOrigenIds } },
            attributes: ["id", "expediente_id", "punto_origen_sesion_id"]
          });

          const todosExpedienteIds = [
            ...new Set(todosLosExpedientes.map((e: any) => e.expediente_id).filter(Boolean))
          ];
    ...
    const checkPageBreak = (y: number, neededHeight: number = 40) => {
      if (y + neededHeight > doc.page.height - 80) {
        doc.addPage();
        drawBackground();
        return 106;
    ...
    if (esExpediente && estudiosInfo.length > 0) {
      estudiosInfo.forEach((est: any, idx: number) => {
        let secY = doc.y;
        let ri = 0;
        secY = drawSectionHeader(
    ...
    if (!esSesion) {
      const comisionesUnicas = [...new Set(
        votosConDetalles.map(v => v.comision_nombre).filter(nombre => nombre && nombre !== 'Sin comisión')
      )].sort();
    ...
    const cortarTexto = (texto: string, max: number): string =>
      texto && texto.length > max ? texto.substring(0, max - 3) + '...' : (texto || '');
...
export const generarPDFAsistencia = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const diputadosAsociadosRaw = await DiputadosAsociados.findAll({
      where: { id_agenda: evento.id },
      raw: true,
    });

    let diputadosAsociadosConDetalles: { nombre: string; partido: string }[] = [];

    if (diputadosAsociadosRaw.length > 0) {
      const asociadosIds = diputadosAsociadosRaw.map((d: any) => d.id_diputado).filter(Boolean);
      const asociadosPartidoIds = diputadosAsociadosRaw.map((d: any) => d.partido_dip).filter(Boolean);

      const diputadosAsoc = await Diputado.findAll({
        where: { id: asociadosIds },
        attributes: ["id", "apaterno", "amaterno", "nombres"],
        raw: true,
      });
      const diputadosAsocMap = new Map(diputadosAsoc.map((d: any) => [d.id, d]));

      const partidosAsoc = await Partidos.findAll({
        where: { id: asociadosPartidoIds },
        attributes: ["id", "siglas"],
        raw: true,
      });
      const partidosAsocMap = new Map(partidosAsoc.map((p: any) => [p.id, p]));

      diputadosAsociadosConDetalles = diputadosAsociadosRaw.map((da: any) => {
        const dip = diputadosAsocMap.get(da.id_diputado);
        const partido = partidosAsocMap.get(da.partido_dip);
        return {
          nombre: dip
            ? `${dip.apaterno ?? ""} ${dip.amaterno ?? ""} ${dip.nombres ?? ""}`.trim()
            : "Sin nombre",
          partido: partido?.siglas || "Sin partido",
        };
      });
    ...
    const drawSectionHeaderAsis = (label: string, y: number) => {
    ...
    if (esSesion) {
      ...
      generarDetalleComisionAsistencia(
        ...
        drawBackground,
        tienetipoReunion ? quorumPorComision : new Map(),
        diputadosAsociadosConDetalles
      );
    }
...
function generarDetalleSesionAsistencia(
  doc: any, 
  asistencias: any[], 
  drawBackground: () => void
) {
  ...
  doc.rect(30, titY, doc.page.width - 60, 22).fill('#96134b');
  doc.fontSize(11).font('Helvetica-Bold').fillColor('#fff')
    .text('DETALLE DE ASISTENCIA', 30, titY + 5, 
      { width: doc.page.width - 60, align: 'center' });
  doc.y = titY + 30;
  ...
  asistenciasOrdenadas.forEach((asist, index) => {
    if (currentY > 700) {
      ...
      doc.rect(30, rY, doc.page.width - 60, 22).fill('#96134b');
      doc.fontSize(11).font('Helvetica-Bold').fillColor('#fff')
        .text('DETALLE DE ASISTENCIA', 30, rY + 5,
          { width: doc.page.width - 60, align: 'center' });
      currentY = rY + 30;
...
function getAsistenciaTexto(sentido: number): string {
  switch (sentido) {
    case 1: return 'ASISTENCIA';
    case 2: return 'ASISTENCIA ZOOM';
...
const escribirTextoConSaltoPagina = (
  doc: any,
  tableW: number,
  drawBackground: () => void,
  texto: string,
  ...
  const words = texto.split(' ').filter((w: string) => w.length > 0);
  const lines: string[] = [];
  let currentLine = '';
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (align === 'justify' && !isLastLine) {
      ...
      if (lineWords.length === 1) {
        ...
        doc.fontSize(8).font('Helvetica');
        const wordWidths = lineWords.map((w: string) => doc.widthOfString(w));
        ...
        let wordX = x;
        lineWords.forEach((word: string, wi: number) => {
          doc.fontSize(8).font('Helvetica').fillColor('#000')
...
export const enviarWhatsAsistenciaPDF = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const intervencionesRaw = await Intervencion.findAll({
      where: { id_evento: id },
      order: [['createdAt', 'ASC']],
      include: [
        {
    ...
    const intervenciones = await Promise.all(
      intervencionesRaw.map(async (inte: any) => {
        const diputado = await Diputado.findOne({
          where: { id: inte.id_diputado },
          attributes: ['apaterno', 'amaterno', 'nombres'],
          raw: true,
    ...
    // ——— Diputados ———
    const diputadoIds = asistenciasRaw.map((a: any) => a.id_diputado).filter(Boolean);
    const diputados = await Diputado.findAll({
      where: { id: diputadoIds },
      attributes: ['id', 'apaterno', 'amaterno', 'nombres'],
      raw: true,
    ...
    // ——— Partidos ———
    const partidoIds = asistenciasRaw.map((a: any) => a.partido_dip).filter(Boolean);
    const partidos = await Partidos.findAll({
      where: { id: partidoIds },
      attributes: ['id', 'siglas'],
      raw: true,
    ...
    if (!esSesion) {
      const comisionIds = asistenciasRaw.map((a: any) => a.comision_dip_id).filter(Boolean);
      if (comisionIds.length > 0) {
        const comisiones = await Comision.findAll({
          where: { id: comisionIds },
          attributes: ['id', 'nombre', 'importancia'],
        ...
        comisionesMap = new Map(comisiones.map((c: any) => [c.id, c]));
      }
 
      const cargoIds = asistenciasRaw.map((a: any) => a.id_cargo_dip).filter(Boolean);
      if (cargoIds.length > 0) {
        const cargos = await TipoCargoComision.findAll({
          where: { id: cargoIds },
          attributes: ['id', 'valor', 'nivel'],
        ...
        cargosMap = new Map(cargos.map((c: any) => [c.id, c]));
      }
    }
 
    // ——— Enriquecer asistencias ———
    const asistenciasConDetalles = asistenciasRaw.map((asistencia: any) => {
      const diputado = diputadosMap.get(asistencia.id_diputado);
      const partido  = partidosMap.get(asistencia.partido_dip);
      const comision = comisionesMap.get(asistencia.comision_dip_id);
      const cargo    = cargosMap.get(asistencia.id_cargo_dip);
    ...
    const totales = {
      asistencia:   asistenciasConDetalles.filter((a: any) => a.asistenciaNumerico === 1).length,
      asistenciaZoom: asistenciasConDetalles.filter((a: any) => a.asistenciaNumerico === 2).length,
      justificada:  asistenciasConDetalles.filter((a: any) => a.asistenciaNumerico === 3).length,
      pendiente:    asistenciasConDetalles.filter((a: any) => a.asistenciaNumerico === 0).length,
    };
    ...
    const quorumPorComision = new Map<string, {
      nombre: string; total: number; asistentes: number;
      requerido: number; tieneQuorum: boolean; importancia: number;
    }>();
    ...
    if (!esSesion && tienetipoReunion) {
      asistenciasConDetalles.forEach((a: any) => {
        ...
        if ([1, 2, 3].includes(a.asistenciaNumerico)) comData.asistentes += 1;
      });
      quorumPorComision.forEach((comData) => {
        comData.requerido   = Math.floor(comData.total / 2) + 1;
        comData.tieneQuorum = comData.asistentes >= comData.requerido;
    ...
    const todasConQuorum =
      quorumPorComision.size > 0 &&
      Array.from(quorumPorComision.values()).every((c) => c.tieneQuorum);
    ...
    if (esSesion) {
      ...
      if (puntosRaw.length > 0) {
        ...
        doc.fontSize(9).font('Helvetica-Bold').fillColor('#fff')
          .text(`PUNTOS A TRATAR — ${puntosRaw.length} PUNTO${puntosRaw.length !== 1 ? 'S' : ''}`,
      ...
      // ===== INTERVENCIONES =====
        if (intervenciones.length > 0) {
          ...
          const renderFichas = (
            ...
            lista.forEach((inte: any, index: number) => {
              ...
              filas.forEach((fila) => {
                const words = fila.texto.split(' ').filter((w: string) => w.length > 0);
                const lines: string[] = [];
                let cur = '';
                for (const word of words) {
              ...
              filas.forEach((fila) => {
                ...
                const yFinal = escribirTextoConSaltoPagina(
                  doc, tableW, drawBackground,
                  fila.texto, valorX, textY, valorW,
                  isJustify ? 'justify' : 'left',
                  continuacionLabel,
                );
          ...
          puntosRaw.forEach((punto: any) => {
            ...
            renderFichas(
              ptoIntervenciones,
              '',
              `PUNTO ${punto.nopunto} (continuación)`,
            );
          });
        }
    ...
    if (!esSesion) {
      const comisionesUnicas = [
        ...new Set(
          asistenciasConDetalles
            .map((a: any) => a.comision_nombre)
            .filter((n: any) => n && n !== 'Sin Comisión'),
        ),
      ].sort();
      if (comisionesUnicas.length > 0) {
        infoComisiones = `\n*Comisiones:*\n${comisionesUnicas.map((c: any) => `- ${c}`).join('\n')}\n`;
      }
    }
 
    const quorumMsg =
      !esSesion && tienetipoReunion
        ? `\n*Quórum:* ${asistentesGeneral}/${totalDiputados} — ${todasConQuorum ? '✅ CON QUÓRUM' : '❌ SIN QUÓRUM'}\n`
        : '';
 
    const mensajeTexto =
      `*ASISTENCIA - ${evento.tipoevento?.nombre || 'Evento'}*\n\n` +
...
export const enviarNotInicioEvento = async (req: Request, res: Response): Promise<any> => {
  try {
    const { id } = req.params;
    ...
    if (!esSesion && tienetipoReunion) {
      asistenciasConDetalles.forEach((a: any) => {
        ...
        const comData = quorumPorComision.get(a.comision_nombre)!;
        comData.total += 1;
        if (a.asistenciaNumerico === 1 || a.asistenciaNumerico === 2) comData.asistentes += 1;
      });
      quorumPorComision.forEach((comData) => {
        comData.requerido = Math.floor(comData.total / 2) + 1;
        comData.tieneQuorum = comData.asistentes >= comData.requerido;
    ...
    console.error("Error completo al generar y enviar PDF de asistencia:", error);
    if (axios.isAxiosError(error)) {
      console.error("Error de Axios:", { message: error.message, code: error.code, response: error.response?.data });
    }
    return res.status(500).json({ 
      message: "Error al generar y enviar PDF de asistencia por WhatsApp",
      error: error.message,
...
function generarDetalleComisionAsistencia(
  ...
  diputadosAsociados: { nombre: string; partido: string }[] = []
) {
  const titY = doc.y;
  doc.rect(30, titY, doc.page.width - 60, 22).fill('#96134b');
  doc.fontSize(11).font('Helvetica-Bold').fillColor('#fff')
    .text('DETALLE DE ASISTENCIA POR COMISIÓN', 30, titY + 5, 
      { width: doc.page.width - 60, align: 'center' });
  doc.y = titY + 30;
  ...
  comisionesOrdenadas.forEach((comision: any) => {
    if (doc.y > 650) {
      ...
      doc.rect(30, rY, doc.page.width - 60, 22).fill('#96134b');
      doc.fontSize(11).font('Helvetica-Bold').fillColor('#fff')
        .text('DETALLE DE ASISTENCIA POR COMISIÓN', 30, rY + 5,
          { width: doc.page.width - 60, align: 'center' });
      doc.y = rY + 30;
    }
  ...
  if (diputadosAsociados.length > 0) {
    ...
    doc.fontSize(9).font('Helvetica-Bold').fillColor('#fff')
      .text('DIPUTADOS ASOCIADOS', 35, titAsocY + 5,
        { width: doc.page.width - 70, align: 'center' });
    doc.y = titAsocY + 20;
    ...
    diputadosAsociados.forEach((da, index) => {
      if (currentY > 700) {
        ...
        doc.rect(30, currentY, doc.page.width - 60, 20).fill('#7a7a7a');
        doc.fontSize(9).font('Helvetica-Bold').fillColor('#fff')
          .text('DIPUTADOS ASOCIADOS (continuación)', 35, currentY + 5,
            { width: doc.page.width - 70, align: 'center' });
        currentY += 20;
